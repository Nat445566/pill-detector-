...
def analyze_pills(image, roi, bg_threshold, min_area):
    ...
    if corner_h < 20 or corner_w < 20: return cropped_image, []
    ...
    if std_dev < bg_threshold: # Uniform Background
        blurred = cv2.GaussianBlur(cropped_image, (7, 7), 0); hsv_image = cv2.cvtColor(blurred, cv2.COLOR_BGR2HSV)
        avg_hsv_bg = np.mean(corners, axis=0); h_range, s_range, v_range = 15, 60, 60
        ...
        mask = cv2.bitwise_not(cv2.inRange(hsv_image, lower_bg, upper_bg))
    ...
    pill_data = []; output_image = cropped_image.copy()

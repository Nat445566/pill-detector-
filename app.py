...
def get_pill_contours(image_cv):
    ...
    st.write("Processing Step: Converting to Grayscale...")
    gray_image = cv2.cvtColor(image_cv, cv2.COLOR_BGR2GRAY)

    # Step 2: Gaussian Blurring (Handbook p. 38) to reduce noise
    st.write("Processing Step: Applying Gaussian Blur...")
    blurred_image = cv2.GaussianBlur(gray_image, (7, 7), 0)

    # Step 3: Otsu's Binarization (Handbook p. 33) to segment pills
    st.write("Processing Step: Applying Otsu's Thresholding...")
    _, binary_mask = cv2.threshold(blurred_image, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)

    # Step 4: Morphological Operations (Practical 7, p. 64-67) to clean the mask
    st.write("Processing Step: Cleaning mask with Morphological Operations...")
    ...
    # Step 5: Find and return the contours of the pills
    st.write("Processing Step: Finding pill contours...")
...
if uploaded_file is not None:
    # Read the file bytes and process it ONCE, saving to session_state
...
if st.session_state.display_image is not None:
    ...
    if analysis_mode == 'Count with ROI (Select a Sample)':
        ...
        canvas_result = st_canvas(
            ...
            stroke_width=3,
            background_image=display_image, # This now correctly uses the persistent image
                ...
                with st.spinner("Analyzing..."):
                    ...
                    if roi_w > 0 and roi_h > 0:
                        ...
                        for cnt in contours:
                            ...
                            # Filter by comparing contour area to the ROI area
        ...
        if st.button("Count All Pills"):
            with st.spinner("Analyzing..."):
                ...
                for cnt in contours:
                    area = cv2.contourArea(cnt)
                    # Filter by comparing contour area to the slider values

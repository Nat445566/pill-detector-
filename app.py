...
def get_color_name(hsv_color):
    ...
    This is a simplified color classifier.
    """
    h, s, v = hsv_color
    
    # Check for white, gray, black first based on Saturation and Value
    if s < 35 and v > 190:
        return "White"
    if s < 35 and v < 80:
        return "Black"
    if s < 45 and 80 <= v <= 190:
        return "Gray"

    # Check for specific colors based on Hue
...
def get_shape_name(contour):
    ...
    if len(approx) > 6: # Likely a smooth curve
...
def analyze_pills(image, roi, bg_threshold, min_area):
    ...
    """
    # Crop image to the selected Region of Interest
    x, y, w, h = roi
    cropped_image = image[y:y+h, x:x+w]
    
    # --- Background Analysis and Masking (Hybrid Strategy) ---
    # Check the standard deviation of color in the corners of the ROI
    # We slice to ensure we don't go out of bounds if ROI is tiny
    ...
    # Choose strategy based on background uniformity
    if std_dev < bg_threshold:
        # STRATEGY 1: Uniform Background
    ...
    for i, cnt in enumerate(contours):
        if cv2.contourArea(cnt) > min_area:
            # Create a mask for just the current pill
            single_pill_mask = np.zeros(cleaned_mask.shape, dtype="uint8")
            cv2.drawContours(single_pill_mask, [cnt], -1, 255, -1)
            
            # Get average color from within the pill's area
            ...
            pill_data.append({
                ...
                "Color": color_name,
                "Shape": shape_name,
                "Area (px)": int(cv2.contourArea(cnt))
            })
            
            # --- Draw results on the output image ---
            cv2.drawContours(output_image, [cnt], -1, (0, 255, 0), 3)
            # Get center of contour to place text
            M = cv2.moments(cnt)
            if M["m00"] != 0:
                cX = int(M["m10"] / M["m00"])
                cY = int(M["m01"] / M["m00"])
                cv2.putText(output_image, str(i + 1), (cX - 15, cY + 15),
...
with st.sidebar:
    ...
    bg_std_threshold = st.slider("2. Background Uniformity Threshold", 5.0, 50.0, 15.0, 
                                 help="Lower if the background is very uniform (like a solid color), higher if it has texture or shadows.")
...
if uploaded_file is not None:
    ...
    with st.sidebar:
        ...
        st.write("Adjust the sliders to select the area to analyze.")
    ...
    with col1:
        ...
        # Draw a rectangle on the original image to show the ROI
